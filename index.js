...
//   trailing slash.
// - forwardMappingTable: DynamoDB table which hold the forwardMapping 
//   minimal table structure {'address' : 'original@example.com', 'forward' : 'forward@example.com'}
var config = {
    'emailBucket': 's3-bucket-name',
    'emailKeyPrefix': 'emailsPrefix/',
    'dbTable' : 'forwardMappingTable',
};
...
exports.handler = function (event, context) {
    ...
    function getForwardEmails(callback){
        var recipients = event.Records[0].ses.receipt.recipients;
        console.log('origin recipients', recipients);
        ...
        params.RequestItems[config.dbTable] = {
            Keys: keys
        };
        var q = docClient.batchGet(params, function(err, data) {
            if (err) {
                console.log(err);
                return err;
            }
            else {
                data.Responses[config.dbTable].forEach(function(item){
                    forwardEmails.push(item.forward); 
                });
                console.log('fowrad to ', forwardEmails);
                callback(forwardEmails);
            }
        });
    }

    function doForward(forwardEmails){
